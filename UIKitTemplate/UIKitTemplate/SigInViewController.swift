...
final class SigInViewController: UIViewController, UITextFieldDelegate {
    ...
    /// мметод для перехода на следующую вью
    @IBAction func saveUserInfo(_ sender: Any) {
        guard let bookingVC = storyboard?.instantiateViewController(identifier: "bookingVC") as? BookingViewController else {
            return
        }
        bookingVC.login = loginTextField.text ?? ""
